...
def printLog2(
    ...
    **kwargs,
    ...
    # global_check
    t = str(datetime.now().strftime("%H:%M"))
    print(        f"|{t}|{t5t1:>2d}|u:{unique:>2d}| Epoch:{global_step:>3d} ", *args, **kwargs)
    if mode != "disabled":
        with open(f"colog/{run_name}.out", "a") as file:
            print(f"|{t}|{t5t1:>2d}|u:{unique:>2d}| Epoch:{global_step:>3d} ", *args, **kwargs, file=file)
...
class Datasetlast10Memory(data.Dataset):
    ...
    def __getitem__(self, item):
        return self.sents_all[self.choice][item], self.augments_all[self.choice][item], item

    def __len__(self):
        return len(self.augments_all[0])
class BoostDatasetlast10Memory(data.Dataset):
    def __init__(self, augments_all, sents_all):
        self.augments_all = augments_all
        self.sents_all = sents_all
        self.org_sents = sents_all[0].copy()
        self.choice = random.choice(list(range(len(self.sents_all))))
        global t5t1
        t5t1 = self.choice
    ...
    def __getitem__(self, item):
        self.choice = random.choice(list(range(len(self.sents_all))))
        return self.sents_all[self.choice][item], self.augments_all[self.choice][item], self.org_sents[item], item
...
class EvalDatasetIterater(data.Dataset):
    def __init__(self, texta, labels):
        self.texta = texta
        self.labels = labels
        assert len(texta) == len(labels)

    def __getitem__(self, item):
        return self.texta[item], self.labels[item]
...
def collect_all_augments(original_sep=False):
    # global t5t1, t5t2
    l = [0, 1, 2, 3, 4, 5]
    # t5t1 = np.random.choice(l)
...
def boost_all(evaldataset, labels, model,  criterion, optimizer, optimizer_head):

    global global_step, metric_list, loss_list, init_epoch_loss
    # save_memory_tensor = lambda t, name: None
    ...
    snn_loss = SNNLCrossEntropy()#model=model
    # scaler = torch.cuda.amp.GradScaler()
    ...
    dataset = BoostDatasetlast10Memory(augments_all, sents_all)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with torch.autograd.set_detect_anomaly(True):
            (
                loss_epoch,
                pseudo_labels
            ) = boost(pseudo_labels=pseudo_labels, data_loader=data_loader, optimizer=optimizer, optimizer_head=optimizer_head, criterion=criterion, snn_loss=snn_loss, scaler=None)#, z_qus, c_qus, max_memory=max_memory)
        ...
        if init_epoch_loss is None and loss_epoch > 0:
            init_epoch_loss = loss_epoch
            # printLog("??????2???\t epoch_loss:", loss_epoch)
        try:
            dic["epoch2first_lss"] = loss_epoch/init_epoch_loss
        except Exception as e:
            print(loss_epoch, init_epoch_loss)
            print(e)
        printLog(f"Epoch {epoch + 1}\t", {k: f"{v:.4f}" for k, v in dic.items()})
        ...
        if pseudo_num > 0:
            score, _ = cluster_utils.clustering_metric(labels[pseudo_index],
                                                       ...
                                                       args.class_num)
            printLog('#Boost NMI={:.2f}||ACC={:.2f}||AVG={:.2f}#Boost'.format(
                                                                        score['nmi'] * 100,
                                                                        #score['ARI'],
                                                                        #score['f_measure'], ARI = {:.4f} F = {:.4f}
                                                                        score['acc'] * 100,
                                                                        (score['nmi'] + score["acc"]) * 100
                                                                        ))
        else:
            printLog("!!!!!!!!!!!!!!!! pseudo_num = 0\t", pseudo_num)
...
def boost(pseudo_labels, data_loader, optimizer, optimizer_head, criterion, snn_loss, scaler=None):
    global unique, init_batch_loss
    n = len(data_loader)
    for step, (x_i, x_j, x, index) in enumerate(data_loader):
        
       
        model.train()
        z  , c   , _, _ = model.single_forward(x,   pooler_type='avg')
        z_i, c_i , _, _ = model.single_forward(x_i, pooler_type='avg')
        z_j, c_j , _, _ = model.single_forward(x_j, pooler_type='avg')
        ...
        loss_epoch, loss  = 0., torch.tensor(0., requires_grad=True).to('cuda')

        # ce_loss = torch.tensor(0., requires_grad=True).to('cuda')
        uni = torch.unique(pseudo_labels[index]).shape[0]
        unique = uni
        soft_nn_loss, ce_loss = None, None
        dic = {
            # "unique"       : unique,
            "batch_loss"   : -1,
            "Soft_NN"      : -1,
        ...
        if unique > class_num:
            # printLog("unique:", unique)
            pseudo_index     = (pseudo_labels[index] != -1)
            selected_labels  = pseudo_labels[index][pseudo_index]
            sel_labels       = selected_labels.repeat(3)
            log_it(selected_labels, len(selected_labels))
            log_it(sel_labels     , len(sel_labels     ))
            sel_embeds       = torch.cat([z, z_i, z_j], dim=0)
            soft_nn_loss     = snn_loss.SNNL(x=sel_embeds, y=sel_labels)
            dic["soft_NN"]   = soft_nn_loss.item()
            loss += soft_nn_loss
            ...
            loss += ce_loss
            dic["CrossEntropy"] = ce_loss.item()

        else:
            print(f"/NO SOFT{step}/", flush=True,  end="\t")
            pair_ins1, pair_clus1 = criterion.forward(z_i, z_j, c_i, c_j)
            pair_ins2, pair_clus2 = criterion.forward(z  , z_j, c  , c_j)
            pair_ins3, pair_clus3 = criterion.forward(z_i, z  , c_i, c  )
            sum = pair_ins1 + pair_clus1 + pair_ins2 + pair_clus2 + pair_ins3 + pair_clus3
            loss += sum
            dic["Pair_instance"] = sum.item()
            # printLog("unique:", unique)

        cluster_instance_loss1 = criterion.forward_instance_elim(z_i, z_j, pseudo_labels[index])
        cluster_instance_loss2 = criterion.forward_instance_elim(z  , z_j, pseudo_labels[index])
        cluster_instance_loss3 = criterion.forward_instance_elim(z_i, z  , pseudo_labels[index])

        

        sum = cluster_instance_loss1 + cluster_instance_loss2 + cluster_instance_loss3
        loss += sum
        dic["clus_instance"] = sum.item()
        idx, counts =  torch.unique(pseudo_labels, return_counts=True)
        log_it(dict(zip(idx.tolist(), counts.tolist())))
        ...
        dic["batch-loss"] = loss.item()
        try:
            dic["batch2first"] = loss.item() / init_batch_loss
        except Exception as e:
            printLog(init_batch_loss, e)
        # "batch/mlm-ls": mlm_loss.item(),
        # "mixcse-ls": mixcse_loss.item(),
           
        if step % (n // 6) == 0: # {int(100 * (step + 1) / n):<2d}%
            try:
                printLog(f"itr {step + 1:3d}/{n} |", {k: f"{v:.3f}" for k, v in dic.items()})
                writer.add_scalars("LOSS/batch", dic, global_step=batch_counter)
            except Exception as e:
        ...
        #     printLog(
        #         f"Step [{step:<3d}/{len(data_loader)}]\t "
        #         f"loss: {loss.item():.4f}\t "
        #         f"SNNL loss: {soft_nn_loss}\t"
        #         f"unique: {unique}", end='///'
        #     )
            print(idx.tolist(), counts.tolist())
                #f"loss_ce: {ce_loss.item()}")
            loss_epoch += loss.item()


    return loss_epoch, pseudo_labels
...
if __name__ == "__main__":
    ...
    if args.resume == True:
        ...
        if True:  # or best_check_path.exists():
            ...
            best_p = r"D:\text_clustering_paper\anydesk\save\Stackoverflow\=q07q4as9=t5_mixcs_ooooooorg\best_model_avg.tar"
    ...
    evaldataset = EvalDatasetIterater(sents, labels)
    ...
    printLog("from epoch {} to epoch {}".format(args.start_epoch, args.epochs))
    try:
        boost_all(evaldataset=evaldataset, labels=labels, model=model, criterion=criterion, optimizer=optimizer, optimizer_head=optimizer)
        # if do_bank is True:
